...
class DepthwiseConv2D(nn.LayerBase):
    ...
    def build_weights(self):
        ...
        weight_shape = (self.kernel_size, self.kernel_size, self.in_ch, self.depth_multiplier)
        if kernel_initializer is not None:
            weight_init = kernel_initializer(weight_shape)
        else:
            weight_init = tf.zeros(weight_shape, dtype=self.dtype)
        ...
        if self.use_bias:
            ...
            bias_shape = (self.in_ch*self.depth_multiplier,)
